        ...
        "recalls": {
            ...
            "categories": {
                "last_month_data": list(
                    recalls.filter(date_de_publication__gte=timezone.now() - timedelta(days=30)).values(
                        'sous_categorie_de_produit').annotate(total=Count('reference_fiche')).order_by('-total')),
                "last_year_data": list(
                    recalls.filter(date_de_publication__gte=timezone.now() - timedelta(days=365)).values(
                        'sous_categorie_de_produit').annotate(total=Count('reference_fiche')).order_by('-total')),
            }
        },
        "dispensations": {
            ...
            "categories": {
                "last_month_data": list(dispensations.filter(datedepot__gte=timezone.now() - timedelta(days=30)).values(
                    'categorie_du_produit_rayon').annotate(total=Count('hash')).order_by('-total')),
                "last_year_data": list(dispensations.filter(datedepot__gte=timezone.now() - timedelta(days=365)).values(
                    'categorie_du_produit_rayon').annotate(total=Count('hash')).order_by('-total')),
            }
        }
    }
