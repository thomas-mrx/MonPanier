...
def search_foods(request, query: str, limit: int = 5, offset: int = 0):
    results = Food.objects.filter(Q(product_name__icontains=query) | Q(brands__icontains=query) | Q(code__startswith=query) | Q(categories__icontains=query))[offset:offset+limit]
    return results